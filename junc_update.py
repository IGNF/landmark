    ...
    for net in tqdm(model.dr_net):
        ...
        if n_pnt_A_th > 1 :
            ...
            if i_passo_dtm == 0:
                ...
                set=0
                ir=i_last           
                for ic in range (j_last, j_slast+1, j_passo):
                    if set == 0:
            ...
            if j_passo_dtm == 0:
                ...
                set = 0
                ic=j_last
                for ir in range(i_last, i_slast, i_passo):
                    if set == 0:
                        if ir>=0 and ir <= model.mat_id.shape[0]-1 and ic>=0 and ic <= model.mat_id.shape[1]-1:
                            if ridge_mask[ir, ic]:
                                model.mat_id[ir, ic].junc = 1
            ...
            if passo_mem != 0:
                ...
                for ir in range(i_last - i_passo, i_slast + i_passo, i_passo):
                    ic = j_last + j_passo
                    if ir>=0 and ir <= model.mat_id.shape[0]-1 and ic>=0 and ic <= model.mat_id.shape[1]-1:
                        if ridge_mask[ir, ic]:
                            model.mat_id[ir, ic].junc = 1
                # nodes placed on the horizontal
                for ic in range(j_last - j_passo, j_slast + j_passo, j_passo):
                    ir=i_last + i_passo
                    if ir>=0 and ir <= model.mat_id.shape[0]-1 and ic>=0 and ic <= model.mat_id.shape[1]-1:
                        if ridge_mask[ir, ic]:
                            model.mat_id[ir, ic].junc = 1
