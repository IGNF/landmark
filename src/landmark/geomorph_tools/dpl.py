...
def dpl(model):
    ...
    for endo in tqdm(model.l_endo_pt, desc="Processing endorheic channels", unit="channel"):
        ...
        main = model.dr_pt[endo.id_pnt-1].id_ch
        # Assign the channel’s endorheic id:
        model.dr_net[main-1].id_endo = endo.id_eo

        # For each drainage point in the main channel:
            
        main_net = model.dr_net[main-1]
        l1 = main_net.length
...
def up_recurs(curr, model):
    ...
    for i in range(model.dr_net[curr-1].n_jun):
        in_curr = model.dr_net[curr-1].id_in[i]
        for cnt_pt in range(model.dr_net[in_curr-1].nel-1): #last pnt belongs to main channel
            dp = model.dr_pt[model.dr_net[in_curr-1].id_pnts[cnt_pt]-1]
            # l1 is the length of the tributary channel.
            l1 = model.dr_net[in_curr-1].length
            l2 = dp.upl
            l3 = model.dr_pt[model.dr_net[in_curr-1].id_end_pt-1].dpl
            model.dr_pt[model.dr_net[in_curr-1].id_pnts[cnt_pt]-1].dpl = l1-l2+l3
            i_curr = dp.i
            j_curr = dp.j
            if dp.fldir is not None and dp.fldir > 0:
                out_dp = model.dr_pt[dp.fldir-1]
                ...
                model.mat_id[i_mat, j_mat] = model.dr_net[in_curr-1].id_ch
        ...
        n_path = model.dr_net[in_curr-1].n_path
        model.dr_net[in_curr-1].id_path.append(model.dr_net[in_curr-1].id_ch)
        for cnt_in in range(2,n_path+1):
            model.dr_net[in_curr-1].id_path.append(model.dr_net[curr-1].id_path[cnt_in-2])
        # Propagate the endorheic id from the parent.
        model.dr_net[in_curr-1].id_endo = model.dr_net[curr-1].id_endo
        
        up_recurs(in_curr, model)
